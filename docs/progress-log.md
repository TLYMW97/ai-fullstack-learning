...
| 3 | 文件上传（腾讯云 COS） | ⬜ | 未开始 |
| 4 | 前端体验与工程化 | ✅ | 设计系统/组件/首页/详情/SSG/ISR/SEO/标签分类/UI 库/404/错误边界全落地（P15–P25） |
| 5 | 部署与服务器运维 | ⬜ | 仅本地 Docker，未上云 |
| 6 | CI/CD（阿里云效 + GitHub + 飞书） | ⬜ | 未开始 |
| 7 | 数据埋点（百度统计） | ⬜ | 未开始 |
| 8 | 收尾与复盘 | 🟡 | 安全自查 + README 已做（P27）；全链路跑通待部署上线后 |
...
- **未做 / 待你定**：
  ...
  3. **旧明文密码已进 git 历史**（`e440b2b` 之前的 P19 记录）。✅ **已换管理员密码**（新密码只单独告知用户、不落盘，旧明文作废）；push 前这条安全事项已消除。
...
### P30 · 修复：极验滑块不显示（product 误用 bind）
- **现象**：登录页滑块无法进行验证（按钮/滑块不渲染），但点登录提示「请先完成滑块验证」。
...
### P35 · 阶段 6 CI/CD：按云效原生方式重构流水线配置
- **动因（用户反馈）**：自写的 `tar` 打包 + 自造解包脚本「很不按流程来」，要求改用云效自带的构建/部署/通知能力，规范配置。
- **查证（官方文档）**：
  - 官方模板为 **「Node.js · 测试、构建、部署到阿里云 ECS/自有主机」**，预置 源/测试/构建/部署 四阶段。
  - **「构建物上传」不是独立可搜索的步骤名**，实际位置是 任务步骤区 → **添加步骤 → 上传 → 构建物上传**（模板的「Node.js 构建上传」任务里已内置），这是之前找不到它的原因。
  - 构建物上传需选 **「归档至云效公共存储空间（供后续任务拉取并部署）」**，配「制品名称 + 打包路径」；勾选「制品中包含打包路径的目录」会保留目录层级（单路径时可不勾，包内即文件本身）。
  - 主机部署任务：制品 / 主机组 / 下载路径 / 执行用户 / 部署脚本 / 部署策略。
  - **通知**：Flow 支持任务插件形式的机器人通知（钉钉机器人通知为官方示例，飞书群同样走 webhook 支持），运行时机可选成功/失败。
- **改动**：
  - 新增 `docs/yunxiao-pipeline.md`：逐步配置文档（模板选择 → 源 → 构建 → 部署 → 通知 → 检查清单 → 风险预案）。
  - `deploy/build.sh`：**删除自造打包**（`tar -czf deploy.tar.gz`），改为只做「装依赖 → prisma generate → next build → 补全 standalone」，产物由云效「构建物上传」归档 `.next/standalone`；固定 `cd` 到仓库根目录以适应 CI 工作目录。
  - `deploy/deploy.sh`：随制品形态简化为**单层解包**（`tar -xzf /root/package.tgz -C /root/blog`），去掉双层包逻辑与手写 curl 通知。
...
### 剩余任务依赖关系（2026-09-07 整理，下次开工看这里）

**已完成**：阶段 0~4 全部 ✅、阶段 2 三种登录 ✅、阶段 3 文件上传 ✅。

**剩余任务分层**（序号 = 建议推进顺序）：

| 序 | 任务 | 依赖 | 需要你提供 |
|---|---|---|---|
| 1 | ~~push 到 GitHub~~ | ~~无~~ | ✅ 已 push（`main` 26 个提交 → origin，远端 `TLYMW97/ai-fullstack-learning`） |
| 2 | 阶段 5 部署（主线） | 无 | 云服务器 + 域名 + 备案 |
| 3 | CDN 加速 | 阶段 5 的备案域名 | 域名（代码已预留 `COS_CDN_DOMAIN`） |
| 3 | 阶段 7 百度统计埋点 | 阶段 5 的域名 | 域名 + 百度账号 |
| 4 | 阶段 6 CI/CD | push + 阶段 5 | GitHub 仓库 + 阿里云效 + 飞书（⏳ 进行中：服务连接/主机组/飞书 webhook 已就绪，流水线按 `docs/yunxiao-pipeline.md` 重建中） |
| 5 | 阶段 8 收尾复盘 | 前面全部 | — |

**依赖链一句话**：`push` 和 `阶段 5 部署` 是两个独立起点；有了域名后，CDN、埋点、CI/CD 都能解锁；最后才做阶段 8 复盘。

**明天从哪开始**：
- ~~有 GitHub 仓库 → 先做「1 push」~~ ✅ 已完成（2026-09-08）
- 准备买服务器 → 直接进「2 阶段 5 部署」这条主线（我可以先给服务器+域名选购方案）
- 都还没准备好 → 先定「买哪家服务器 + 什么域名」，我再帮你理部署全流程
